...
function RSSolve(RSScript, RSURL, RSUA){
    ...
    sdbx.defineProperty(Document.prototype, 'createElement', {writable: true, enumerable: true, configurable: true,
        value(ele){
            ...
            switch (ele) {
                ...
                case 'script':
                    debugger
                    newEle = new HTMLScriptElement('new')
                    newEle.tagName = "SCRIPT";
                    newEle.innerHTML= "";
                    newEle.outerHTML = "<script></script>";
                    break;
                default:
                    debugger
    ...
    sdbx.defineProperty(Document.prototype, 'getElementsByTagName', {writable: true, enumerable: true, configurable: true,
        value(name){
            ...
            switch (name) {
                ...
                case 'head':
                    debugger
                    let head = new HTMLHeadElement('new');
                    head = sdbx.proxy(head, `document.getElementsByTagName.${name}.${name}1`)
    ...
    localStorage.setItem('$_ff', "WOY3tgjBxqZ7nw0L0Nj3_U8x6r0xj0Aexv49yC7BXbL5LdUj8qVhfXJLpKMF.lRG")
...
function env(RSScript, RSURL, RSUA){
    try {
        ret = RSSolve(RSScript, RSURL, RSUA)
        return {
            'status': true,
            'data': ret,
        }
...
fs = require('fs');
script6 = fs.readFileSync(`${__dirname}/src/script6.js`, encoding='utf-8')
console.time('耗时')
result = env(
    RSScript=script6,
    RSURL='https://www.urbtix.hk/api/internet/promotion/promotionFeature/banners',
